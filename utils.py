...
class TrainerWithCustomSampler(Trainer):
    def __init__(self, *args, loss_threshold=0.2, **kwargs):
    ...
    def log(self, logs: Dict[str, float]) -> None:
        super().log(logs)
        # 只在 eval_loss 存在并且低于 threshold 时保存 checkpoint
        if 'eval_loss' in logs and logs['eval_loss'] < self.loss_threshold:
            print(f"[INFO] Saving checkpoint at step {self.state.global_step} (eval_loss={logs['eval_loss']:.4f})")
            self.save_model(output_dir=os.path.join(self.args.output_dir, f"step_{self.state.global_step}"))
